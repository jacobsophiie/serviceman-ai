 ...
 */
/** Long trade names would push the strip onto two lines — shorten them. */
const shortLabels: Record<string, string> = {
  "air-conditioning technicians": "air-con techs",
  "pest controllers": "pest control",
};

export function TrustBar({ tradesLabel = "tradies" }: { tradesLabel?: string }) {
  const points = [
    "Free to post a job",
    "Takes 2 minutes",
  ...
  return (
    <ul className="flex flex-wrap items-center gap-x-4 gap-y-2">
      {points.map((point) => (
        <li
          key={point}
          className="flex items-center gap-1.5 whitespace-nowrap text-[13px] font-medium text-ink"
        >
